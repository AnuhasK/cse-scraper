...
INPUT_JSON_DIR = os.path.join(PROJECT_ROOT, "output")
OUTPUT_PDF_DIR = os.path.join(PROJECT_ROOT, "pdfs")

# --- Filtering & Network Configuration ---
CUTOFF_TIMESTAMP = 1771977600000  # Feb 25, 2026, in Unix Epoch milliseconds
...
def download_pdf(path, base_ticker, category, report_date, logger):
    ...
    # NEW: Create the category-specific directory: financials-json/pdfs/[TICKER]/[CATEGORY]/
...
def process_json_file(filepath, logger):
    ...
    # Iterate through the arrays and apply the temporal filter
    for key, category in report_categories.items():
        for item in data.get(key, []):
            uploaded_date = item.get('uploadedDate')
            
            # Skip if missing or uploaded before our cutoff threshold
            if uploaded_date and uploaded_date > CUTOFF_TIMESTAMP:
                path = item.get('path')
...
def main():
    ...
    logger.info("Scanning for reports uploaded after February 25, 2026...")
